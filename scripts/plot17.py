...
"""
def main():
	args = parse_args("plot17")
	if args:

		col_name= 'daily_vaccinations_per_million'
		ylabel='Daily number of vaccinations (per M)'
		ptitle='Daily number of vaccinations (per M of citizens)'
	...
	#			print(f"x = {xdata},\n y ={ydata},\nmask = {xmask}")
				ax.text((delta_w_start+omi_w_start)/2, yval[-1]+1e3 ,f"Delta\nWave", size=15, va="center", ha="center",)
				ax.text((omi_w_end+omi_w_start)/2, yval[-1]+1e3 ,f"Omicron Wave", size=15, va="center", ha="center",)
	...
	#			ax.set_xticks(years_idx,use_names,size=15)
				...
				t=ax.text(dates[-200], yval[-2] ,f"{xs[frame]}", size=25, va="center", ha="left",)
