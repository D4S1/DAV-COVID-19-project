...
def main():
	...
	if args:
		if args[1]<3 :
			...
			fig = go.Figure()
			fig.add_trace(go.Scatter(x=data["date"].values, y=data["total_deaths"].values,
					mode='lines',
					name='Australia (weekly)'))
			data=pd.read_csv(args[2])
			fig.add_trace(go.Scatter(x=data["date"].values, y=data["deaths_cum"].values,
					mode='lines',
...
#			print(df['ix']==155)
		...
		fig.update_layout(
			...
			yaxis=dict(
				title=dict(text='Cumulative number of  COVID-related deaths'),
				tickfont = dict(size=15)
			),
			legend=dict(
				title=dict(
			...
			title=dict(
				text='Total number of COVID - related deaths (cumulative)',
				font=dict(size=30),
				x=0.5,
				xanchor='center',
			),			
