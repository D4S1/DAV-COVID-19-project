...
#					fill='tonexty',
			...
			values=[data["cumulative_estimated_daily_excess_deaths"].values]
			CI = [data[["cumulative_estimated_daily_excess_deaths_ci_95_bot","cumulative_estimated_daily_excess_deaths_ci_95_top"]].values]
			data=pd.read_csv(args[9])
			ymin=min(data["cumulative_estimated_daily_excess_deaths"].min(),ymin)
			ymax=max(ymax,data["cumulative_estimated_daily_excess_deaths"].max())
			values.append(data["cumulative_estimated_daily_excess_deaths"].values)
			names=["Australia","Poland"]
			code = ["AUS","POL"]
			CI.append(data[["cumulative_estimated_daily_excess_deaths_ci_95_bot","cumulative_estimated_daily_excess_deaths_ci_95_top"]].values)
	...
	#		print(f"number of steps: {len(dates)}\nshape of values:\t{values[0].shape}\t{values[1].shape}\nshape of CI:\t{CI[0].shape}\t{CI[1].shape}\nymin = {ymin}\tymax = {ymax}")
			...
			for i in range(5):
				yval.append(i*50000)
				if i:
					ylab.append(f"{50*i}k")
			def init():
				ax.set_ylim(ymin, ymax)
				ax.set_xlim(dates[0],dates[-1]+60)
#				ax.set_yticks(np.array(valmax), valmax,size=12)
#				ax.set_xticks(years_idx,use_names,size=15)
				ax.set_xlabel('Total excess of COVID - related deaths (compared to normal times, cumulative)',size=18)
				ax.set_ylabel('Cumulative excess of COVID-related deaths',size=18)
				del xdata[:]
...
#				print(f"x = {xdata}\ny = {ydata}\nCI: {yCIdata}")
				...
				ax.set_title('Total excess of COVID - related deaths\n(compared to normal times, cumulative)',size=20)
	#			ax.set_xticks(years_idx,use_names,size=15)
