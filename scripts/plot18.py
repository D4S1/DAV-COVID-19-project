...
"""
def main():
	args = parse_args("plot18")
	if args:

		col_name= 'people_vaccinated_per_hundred'
		ylabel='Total percentage of people vaccinated in country'
		ptitle='Total percentage of people vaccinated'
		...
		if args[1]<3 :
			...
			fig = go.Figure()
			fig.add_trace(go.Scatter(x=data["date"], y=data[col_name]/100,
					mode='lines+markers',
					name='Australia',
...
#			print(f"number of steps: {len(dates)}\nshape of values:\t{values[0].shape}\t{values[1].shape}\n\nymin = {ymin}\tymax = {ymax}")
	...
	#			print(f"x = {xdata},\n y ={ydata},\nmask = {xmask}")
				ax.text((delta_w_start+omi_w_start)/2, yval[-2] ,f"Delta\nWave", size=15, va="center", ha="center",)
				ax.text((omi_w_end+omi_w_start)/2, yval[-2] ,f"Omicron Wave", size=15, va="center", ha="center",)
	...
	#			ax.set_xticks(years_idx,use_names,size=15)
				...
				t=ax.text(dates[-200], yval[2]+5 ,f"{xs[frame]}", size=25, va="center", ha="left",)
