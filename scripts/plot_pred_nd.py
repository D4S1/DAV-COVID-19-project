...
def main():
	args = parse_args("plot_pred_nd")
	if args:
		# common parts
		data=pd.read_csv(args[18])[["date","Australia"]]
...
#		print(pred_summary)
		...
		s=0
		d=0
		model = ARIMA(df_diff2,order=(p,s,d),trend="c")
		model_fit = model.fit() 
...
#		print(pred_summary2)
	...
	#				hovertext='Start of prediction 2,\n2023-01-04',
			...
			fig.update_layout(
				...
				yaxis=dict(
					title=dict(text='New deaths per milion'),
					tickfont = dict(size=15)
				),
				legend=dict(
					title=dict(
				...
				title=dict(
					text='Daily deaths per milion of population',
					font=dict(size=30),
					x=0.5,
					xanchor='center',
				),			
