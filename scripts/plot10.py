...
def main():
	args = parse_args("plot10")
	if args:
		if args[1]<3 :
			
			data=pd.read_csv(args[17])
			...
			fig.update_layout(
				...
				title=dict(
					text='Total number of cases per milion of population',
					font=dict(size=30),
					x=0.5,
					xanchor='center',
				),			
...
#			fplot2 = ax.fill_between(np.array(dates[0]),np.array([0]),np.array([0]))
			...
			ylab=["0"]
			for i in range(5):
				yval.append(i*100000)
				if i:

					ylab.append(f"{i*100}k")
			def init():
...
#					print(f"frame = {frame},\ti = {i}\ntmp = {tmp.shape}\nmask = {mask.shape}\ndata={len(xdata)}\nnewY = {tmp[mask]}")
				...
				ax.set_title('Total number of cases per milion of population',size=20)
	#			ax.set_xticks(years_idx,use_names,size=15)
				...
				t=ax.text(dates[5], yval[-1]*0.83 ,f"{xs[frame]}", size=25, va="center", ha="left",)
