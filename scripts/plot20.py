...
def main():
	args = parse_args("plot20")
	if args:

		ylabel="Tatal number of vaccinations"
		ptitle="Tatal number of vaccinations by state of Australia"
		data=pd.read_csv(args[12])
		todrop=[f"{key}_new" for key in list(codes.keys())]
		torename=dict(zip([f"{key}_cum" for key in list(codes.keys())],list(codes.values())))
		...
		if args[1]<3 :
			...
			def update(frame):
				...
				ax.text((delta_w_start+omi_w_start)/2, yval[-1]*1.1 ,f"Delta\nWave", size=15, va="center", ha="center",)
				ax.text((omi_w_end+omi_w_start)/2, yval[-1]*1.1 ,f"Omicron Wave", size=15, va="center", ha="center",)
				for i in range(len(ydata)):
				...
				t=ax.text(dates[10], yval[-1]-1e6 ,f"{xs[frame]}", size=25, va="center", ha="left",)
