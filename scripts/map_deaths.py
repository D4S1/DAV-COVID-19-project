...
if __name__ == "__main__":
    ...
    df_states = df_states[['date', 'state', 'deaths_cum']]
    ...
    cases_min = df_states['deaths_cum'].min()
    cases_max = df_states['deaths_cum'].max()
    ...
    colormap.caption = 'Number of COVID related deaths'
    ...
    for _, state in enumerate(geo_df.STATE_NAME):
        ...
        df = pd.DataFrame(
            {
                "color": df_state.deaths_cum.apply(colormap).values,
    ...
    if args.mode == 0:
        ...
        output_path = "/home/hoshi/ME/Coding/DAV-COVID-19-project/images/map_deaths.html"
        m.save(output_path)
