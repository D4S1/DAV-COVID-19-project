...
def main():
	args = parse_args("plot13")
	if args:

		col_name='total_vaccinations'
		ylabel='Total number of doses'
		ptitle='Total number of doses administered'
		...
		if args[1]<3 :
			...
			fig = go.Figure()
			fig.add_trace(go.Scatter(x=data["date"].values, y=data[col_name].values,
					mode='lines+markers',
					name='Australia',
...
#			print(f"number of steps: {len(dates)}\nshape of values:\t{values[0].shape}\t{values[1].shape}\n\nymin = {ymin}\tymax = {ymax}")
			...
			def init():
				...
				for i in range(len(ydata)):
					del ydata[i][:]
					del xmask[i][:]
				t = ax.text(dates[65], yval[2]*0.74  ,xs[0], size=30, va="center", ha="center",)
				return lplot,t
	...
	#			ax.set_xticks(years_idx,use_names,size=15)
				...
				t=ax.text(dates[-180], yval[2]*0.8 ,f"{xs[frame]}", size=25, va="center", ha="left",)
